...
def build_greater_london_boroughs():
    categorisation_service.create_categorisation_with_values(
...
def build_colours():
    categorisation_service.create_categorisation_with_values("C1", "Colours", "Paint", "Cars", values=["Red", "Black"])
    categorisation_service.create_categorisation_with_values("C2", "Colours", "Paint", "Nails", values=["Pink", "Red"])
...
def test_add_category_to_dimension_does_append(db_session, stub_page_with_dimension):
    ...
    categorisation_service.link_categorisation_to_dimension(
        dimension, categorisation, includes_parents=False, includes_all=False, includes_unknown=False
    )
...
def test_link_category_to_dimension_does_append(db_session, stub_page_with_dimension):
    ...
    categorisation = categorisation_service.get_categorisation_by_code("L1")
    categorisation_service.link_categorisation_to_dimension(
        dimension, categorisation, includes_parents=False, includes_all=True, includes_unknown=False
    )
...
def test_link_category_to_dimension_does_save_data_properties(db_session, stub_page_with_dimension):
    ...
    categorisation = categorisation_service.get_categorisation_by_code("L1")
    categorisation_service.link_categorisation_to_dimension(
        dimension, categorisation=categorisation, includes_parents=False, includes_all=True, includes_unknown=False
    )
...
def test_get_category_from_dimension_by_family_does_get_correct_category(db_session, stub_page_with_dimension):
    ...
    categorisation_service.link_categorisation_to_dimension(
        dimension, categorisation=greater_london, includes_parents=False, includes_all=True, includes_unknown=False
    )
    car_colours = categorisation_service.get_categorisation_by_code("C1")
    categorisation_service.link_categorisation_to_dimension(
        dimension, categorisation=car_colours, includes_parents=False, includes_all=True, includes_unknown=False
    )
    # when we request
    greater_london_expected = categorisation_service.get_categorisation_link_for_dimension_by_family(
        dimension, "Geography"
    )
...
def test_link_category_to_dimension_does_remove_link(db_session, stub_page_with_dimension):
    ...
    # when
    categorisation_service.unlink_categorisation_from_dimension(
        dimension=stub_page_with_dimension.dimensions[0], categorisation=greater_london
    )
...
def test_add_value_to_category_appends_new_value(db_session):
    ...
    categorisation_service.add_values_to_categorisation(greater_london, ["Barnet", "Camden", "Haringey"])
    categorisation_service.add_values_to_categorisation(inner_london, ["Camden", "Haringey"])
...
def test_remove_value_from_category_removes_value(db_session):
    # given a setup with one
    greater_london = categorisation_service.create_categorisation(
        "G1", "Geography", "Local level", "Greater London Boroughs"
    )
    inner_london = categorisation_service.create_categorisation(
...
def test_add_parent_value_to_category_appends_new_parent(db_session):
    ...
    rdu = categorisation_service.create_categorisation_with_values(
        "G1", "People", "Teams", "Race Disparity Unit", values=people
    )
    rdu_by_tribe = categorisation_service.create_categorisation_with_values(
        "G2", "People", "Teams", "Race Disparity Unit by Tribe", values=people
    )
    rdu_by_gender = categorisation_service.create_categorisation_with_values(
