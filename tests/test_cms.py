...
def test_create_measure_page(client, mock_user, mock_create_page, mock_get_page, stub_topic_page, stub_measure_page):
    ...
    resp = client.post(url_for('cms.create_measure_page', topic_slug=stub_topic_page.guid),
                       data=form_data,
...
def test_reject_page(client, mock_user, stub_topic_page, mock_reject_page):
    with client.session_transaction() as session:
        session['user_id'] = mock_user.id
